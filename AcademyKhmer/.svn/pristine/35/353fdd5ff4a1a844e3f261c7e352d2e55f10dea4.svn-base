	...
	if (json.length != 0) {
		for (var i = 0; i < json.length; i++) {
			...
			history += "<div class='the-box no-border store-list'>"
					...
					+ " <li><a href='../elearning/play.act?v="+json[i].videoid+"'>"+json[i].videoname+"</a></li>"
							+"</br>"
					+ " <li><a href='../admin/user.jsp?v="+json[i].userid+"'> by "+json[i].username+"</a></li>"
					+ "</ul>"
					+ "<p class='hidden-xs'> "+json[i].videodescription+"</p>"
					+"<span class='small text-muted' style='color:#37BC9B'><i class='fa fa-eye'>&nbsp;&nbsp;"+json[i].videoviewcount+"</i>      &nbsp;&nbsp;&nbsp; </span>"
					+ "</div>"
					+ "</div>"
					+ "</div>";
		}
