...
public class ActionListVideotToAddToPlaylist implements Action {
	...
	public ActionForward execute(HttpServletRequest request,
		...
		ResultSet rs = dao.listAllVideoToAddToPlaylist(pagin, dto.getVideoname());
		pagin.setTotalCount(dao.countVideoToAddToPlaylist( dto.getVideoname() ));
		while (rs.next()) {
			dto =  new Video(rs.getInt("videoid"), rs.getString("videoname"), rs.getString("description"), rs.getString("youtubeurl"), rs.getString("fileurl"), rs.getBoolean("publicview"), rs.getDate("postdate"), rs.getInt("userid"), rs.getInt("viewcount"));
		...
		String listvideosearch_json = new Gson().toJson(hash);
		System.out.println("listvideosearch_json "+ listvideosearch_json);
		response.getWriter().write(listvideosearch_json);
